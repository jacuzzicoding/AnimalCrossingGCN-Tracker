...
struct HomeView: View {
    ...
    // Convenience initializer that creates a default viewModel from dependencies
    init() {
        // This will be resolved during view initialization when dependencies are available
        _viewModel = StateObject(wrappedValue: HomeViewModel.placeholder)
    }
    ...
    var body: some View {
        ...
        .onAppear {
            // Configure viewModel with dependencies if it's a placeholder
            if viewModel === HomeViewModel.placeholder {
                do {
                    let resolvedAnalyticsService = try dependencyContainer.resolve(AnalyticsServiceProtocol.self)
                    let resolvedDonationService = try dependencyContainer.resolve(DonationServiceProtocol.self)
                    viewModel.configure(
                        dataManager: dataManager,
                        analyticsService: resolvedAnalyticsService,
                        donationService: resolvedDonationService
                    )
                } catch {
                    print("Failed to resolve dependencies for HomeViewModel: \(error)")
                }
            }
